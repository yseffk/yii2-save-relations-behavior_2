...
class SaveRelationsBehavior extends Behavior
{
    ...
    {
        parent::init();
        $allowedProperties = ['scenario', 'extraColumns', 'cascadeDelete','options'];
    ...
    public function _afterSaveHasManyRelation($relationName)
    {
        ...
        foreach ($owner->{$relationName} as $i => $relationModel) {
            //echo 'entr in array0<br/>';
            if ($relationModel->isNewRecord) {
                //echo 'entr in if true<br/>';
        ...
        if(isset($this->_relationsOptions[$relationName]['ignoreUniqueInsert']) && $this->_relationsOptions[$relationName]['ignoreUniqueInsert']===true){
            foreach ($initialRelations as $model) {
                $junctionTableColumns = $this->_getJunctionTableColumns($relationName,$model);
                $owner->link($relationName, $model, $junctionTableColumns);
            }
        }else{
